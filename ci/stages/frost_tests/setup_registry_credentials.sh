#!/bin/bash
# Runs ON THE LOGIN NODE, before srun. Frost-local copy of
# ci/stages/oss_tests/setup_registry_credentials.sh: everything the frost jobs
# execute lives under ci/stages/frost_tests/, so this stage can be changed
# without reasoning about the oss one. Finds a credential pair that the GitLab
