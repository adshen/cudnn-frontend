...
from cudnn.sdpa.fwd.config_sm100 import cga_for_flavor, pack_gqa_supported
...
class Capabilities:
    ...
    # Shape-specific domains for an engine that lowers several native flavors.
    # The default ``cgas`` applies unless the smallest covering native shape is
    # listed here.  Keep this immutable: Capabilities is frozen and plan probes
    # must see one stable domain for a given graph shape.
...
def effective_cgas(capabilities: Capabilities, facts: "ga.SdpaGraphFacts") -> frozenset[int]:
    """CGA domain of the native flavor selected for ``facts``.

    Engine rows are architecture x dtype-family strategies, not head-dimension
    identities.  Most flavors share the row's default domain, while D256 FP8
    uses an independent CTA1 pipeline.  Resolve that conditional domain before
    probing or proposing a plan so a knob is never accepted and then ignored.
    """

    selected = _selected_d_shape(capabilities, facts)
    if selected is None:
        return capabilities.cgas
    for shape, domain in capabilities.cgas_by_d_shape:
        if shape == selected:
            return domain
    return capabilities.cgas
...
def mismatch(capabilities: Capabilities, facts: "ga.SdpaGraphFacts", knobs: Optional[SdpaFwdKnobs] = None) -> Optional[str]:
    ...
    if knobs is not None:
        ...
        for value, domain, label in (
            ...
            (knobs.cga, effective_cgas(capabilities, facts), "cga"),
...
def _sm100_mxfp8_spec() -> EngineSpec:
    ...
    return EngineSpec(
        ...
        capabilities=Capabilities(
            ...
            d_pad_multiple=0,
            # D128 and D256 carry both the write_thd_meta THD leg and
            # split-major partial O/LSE plumbing. D192/D128 remains dense-only.
            thd_d_shapes=frozenset({(128, 128), (256, 256)}),
            split_d_shapes=frozenset({(128, 128), (256, 256)}),
            ...
            tile_ns=frozenset({128}),
            cgas=frozenset({2}),
            cgas_by_d_shape=(((256, 256), frozenset({cga_for_flavor((256, 256), fp8=True)})),),
...
def _sm100_fp8_spec(*, arch: str = "sm100") -> EngineSpec:
    ...
    - split_kv_supported / split_d_shapes: the SM100 d128 and d256 kernels wire
      SplitHelpers, and the SM107 d128 sibling carries the same plumbing as its
      SM100 twin; d192x128 remains unsplit.
    ...
    return EngineSpec(
        ...
        capabilities=Capabilities(
            ...
            tile_ns=frozenset({128}),
            cgas=frozenset({2}),
            cgas_by_d_shape=() if rubin_row else (((256, 256), frozenset({cga_for_flavor((256, 256), fp8=True)})),),
            ...
            # Split partials reduce in half precision, so mismatch()'s facts x
            # knobs gate additionally requires a bf16/fp16 O. Both arch rows
            # wire D128; pre-Rubin SM100 also wires D256. D192/D128 remains
            # unsplit.
            split_kv_supported=True,
            split_d_shapes=(frozenset({(128, 128)}) if rubin_row else frozenset({(128, 128), (256, 256)})),
            pack_gqas=frozenset({False, True}),
        ),
        lower=partial(lower_dsl_prefill, api_type=_SM100),
    )
