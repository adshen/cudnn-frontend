...
def _exp2_dense_chunk_a(vec, softmax_half):
    ...
    for i in range(0, int(vec.shape[0]), 2):
        ...
        use_e5_poly = _E5_STYLE_SOFTMAX and (
            i % 10 < 4
            or (softmax_half == 1 and (i == 24 or i == 26))
            or (CFG.DTYPE_QKV == 0 and softmax_half == 0 and (i == 26 or i == 28))
        )
        if use_e4_poly:
...
def _exp2_dense_chunk_b(vec, softmax_half):
    ...
    for i in range(0, int(vec.shape[0]), 2):
        ...
        if (not _E5_STYLE_SOFTMAX and i >= e4_threshold) or (_E5_STYLE_SOFTMAX and i >= e5_threshold):
            x, y = ex2_emulation_2(
                vec[i], vec[i + 1], poly_degree=1 if CFG.DTYPE_QKV == 0 else 2
            )
        else:
            x = cute.math.exp2(vec[i], fastmath=True)
            y = cute.math.exp2(vec[i + 1], fastmath=True)
...
def _exp2_masked_tail_chunk_a(vec, softmax_half):
    ...
    for i in range(0, int(vec.shape[0]), 2):
        use_poly = (
            i % 10 < 4
            or (softmax_half == 1 and (i == 24 or i == 26))
            or (softmax_half == 0 and (i == 26 or i == 28))
        )
        if use_poly:
...
_STAT_FULL_WARP_ARRIVALS = (
    CFG.SOFTMAX_WARPGROUPS == 2
...
def _tmaldg_warp_group(
    ...
    if cutlass.const_expr(CFG.THD_VARLEN):
        k_rt_ptr = (o_desc_words.iterator.raw_ptr() + (n_batch + cutlass.Int32(1)) * cutlass.Int32(_TENSOR_MAP_QWORDS)).tospace(
            cutlass.AddressSpace.generic
        )
        v_rt_ptr = (o_desc_words.iterator.raw_ptr() + (n_batch + cutlass.Int32(2)) * cutlass.Int32(_TENSOR_MAP_QWORDS)).tospace(
            cutlass.AddressSpace.generic
        )
...
def _tmastg_warp_group(
    ...
    while is_valid_tile > cutlass.Int32(0):
        ...
        if cutlass.const_expr(CFG.THD_VARLEN):
            o_desc_ptr = (
                o_desc_words.iterator.raw_ptr() + batch_idx * cutlass.Int32(_TENSOR_MAP_QWORDS)
            ).tospace(cutlass.AddressSpace.generic)
        else:
        ...
        for chunk in cutlass.range_constexpr(N_O_CHUNKS):
            ...
            if cutlass.const_expr(CFG.THD_VARLEN):
                ...
                nvvm.cp_async_bulk_tensor_global_shared_cta(
                    o_desc_ptr,
                    smem_chunk,
                    (o_slice.coord_d + d_coord, head_idx, q_row_coord, batch_idx),
                )
            tma_store_commit()
            ...
            if cutlass.const_expr(CFG.MASK_FLAGS != MASK_NONE):
                for kv_loop in cutlass.range(bounds.unmasked_hi, bounds.right, 1, unroll=1):
                    ...
                    if cutlass.const_expr(softmax_half == 0):
                        ...
                        exchange_base = parity_rt * cutlass.Int32(2 * CFG.TILE_M)
                        softmax_exchange.subview(
                            exchange_base + cutlass.Int32(CFG.TILE_M) + tid_in_wg
                        ).store(total_max_safe)
                        mb_softmax_max[parity_rt].arrive()
                    else:
        ...
        for block_idx in cutlass.range_constexpr(N_BLOCKS_EPI):
            for sub in cutlass.range_constexpr(CHUNKS_PER_BLK):
                ...
                if cutlass.const_expr(CFG.THD_VARLEN):
                    ...
                    invalid = row_dead | (~_row_valid)
                    o_scaled = cutlass.Vector.from_elements(
                        tuple(
                            cutlass.Float32(
                                arith.select(invalid.ir_value(), zero.ir_value(), o_scaled[i].ir_value())
                            )
...
def compile(  # noqa: A001
    ...
    fake_o = _fake_bshd((fake_batch, sq, qh, d_v), o_stride, dtype=OUT_STORAGE_DTYPE, bpe=CFG.BPE_O)
    if not has_lse:
    ...
    fake_seq_kv_lens = cute.runtime.make_fake_compact_tensor(
        cutlass.Int32,
        ((3 * b + 2) if CFG.THD_VARLEN else b,),
        stride_order=(0,),
        assumed_align=16,
    )
