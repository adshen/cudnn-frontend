...
def _kernel(
        ...
        while is_valid != 0:
            ...
            for k_tile_idx in range(num_k_tiles):
                ...
                for _ai in cutlass.range_constexpr(num_a_operands):
                    ...
                    sSFA_stage = smem_sfa_list[_ai] + sfa_smem_bytes * stage
                                ...
                                if cutlass.const_expr(a_is_m_major):
                                    for m_group in cutlass.range_constexpr(cta_tile_mnk[0] // a_tma_group_elems):
                                        nvvm.cp_async_bulk_tensor_shared_cluster_global(
                                            sA_stage + m_group * a_tma_group_elems * cgrp_tile_mnk[2],
                                            tma_a_desc.get_ptr(),
                                            (
                ...
                for _bj in cutlass.range_constexpr(num_b_operands):
                    ...
                    sSFB_stage = smem_sfb_list[_bj] + sfb_smem_bytes * stage
                            ...
                            if nvvm.elect_sync():
                                if cutlass.const_expr(b_is_n_major):
                                    for n_group in cutlass.range_constexpr(cta_tile_mnk[1] // b_tma_group_elems):
                                        nvvm.cp_async_bulk_tensor_shared_cluster_global(
                                            sB_stage + n_group * b_tma_group_elems * cgrp_tile_mnk[2],
                                            tma_b_desc.get_ptr(),
                                            (
    ...
    if warp_idx == mma_warp_id:
        ...
        if is_pair_leader:
            ...
            idesc_by_j = [
                cutlass.primitives.Tcgen05MxInstrDesc.build(
            ...
            while is_valid != 0:
                ...
                for k_tile_idx in range(num_k_tiles):
                    ...
                    desc_a_bases = [
                        cutlass.primitives.Tcgen05SmemDesc.build(
                            start_address=smem_a_list[i] + sA_elems * stage,
                    ...
                    desc_b_bases = [
                        cutlass.primitives.Tcgen05SmemDesc.build(
                            start_address=smem_b_list[j] + sB_elems * stage,
                    ...
                    desc_sfa_bases = [
                        cutlass.primitives.Tcgen05SmemDesc.build(
                            start_address=smem_sfa_list[i] + sfa_smem_bytes * stage,
                            leading_byte_offset=16,
                            stride_byte_offset=128,
                            layout=cutlass.primitives.Tcgen05SmemSwizzle.NONE,
                        )
                        for i in range(num_a_operands)
                    ]
                    desc_sfb_bases = [
                        cutlass.primitives.Tcgen05SmemDesc.build(
                            start_address=smem_sfb_list[j] + sfb_smem_bytes * stage,
                            leading_byte_offset=16,
                            stride_byte_offset=128,
                            layout=cutlass.primitives.Tcgen05SmemSwizzle.NONE,
                        )
...
def _host(
    ...
    c_stride_m = problem_size[10]
    c_stride_n = problem_size[11]
    c_stride_l = problem_size[12]
    ...
    for _a_op, _sfa_op in zip(_a_operands, _sfa_operands):
        ...
        tma_sfa_desc_list.append(
            _tma.create_tensor_map_tiled_from_tensor(
                sfa_fp16_tensor,
...
def compile() -> Callable:
    ...
    sym_k = cute.sym_int64(divisibility=cta_tile_k_elems)
    sym_kp = cute.sym_int64(divisibility=ab_packed_per_row)
    sym_ksf = cute.sym_int64(divisibility=sf_k)
