    ...
    if warp_idx == 0:
        if nvvm.elect_sync():
            for i in range(ab_stages):
                ...
                nvvm.mbarrier_init(mainloop_full_mbar_ptr + i, num_mainloop_warps)
                            ...
                            if cutlass.const_expr(a_is_m_major):
                                for m_group in cutlass.range_constexpr(cta_tile_mnk[0] // a_tma_group_elems):
                                    nvvm.cp_async_bulk_tensor_shared_cluster_global(
                                        sA_tma_dst + m_group * a_tma_group_elems * cta_tile_mnk[2],
                                        tma_a_desc.get_ptr(),
                                        (
                        ...
                        if nvvm.elect_sync():
                            if cutlass.const_expr(b_is_n_major):
                                for n_group in cutlass.range_constexpr(cta_tile_mnk[1] // b_tma_group_elems):
                                    nvvm.cp_async_bulk_tensor_shared_cluster_global(
                                        sB_tma_dst + n_group * b_tma_group_elems * cta_tile_mnk[2],
                                        tma_b_desc.get_ptr(),
                                        (
    ...
    if warp_idx >= mainloop_warp_id_start:
        ...
        while is_valid != 0:
            for k_tile_idx in range(num_k_tiles):
                ...
                if cutlass.const_expr(mainloop_fuse_a):
                    ...
                    if cutlass.const_expr(mainloop_a_cast):
                        ...
                        for chunk in cutlass.range_constexpr(ml_chunks_a):
                            ...
                            ml_kloc = cutlass.full_like(ml_f32_a, cutlass.Float32(ml_L % cta_tile_mnk[2]))
                ...
                cute.arch.sync_warp()
                if nvvm.elect_sync():
                    nvvm.mbarrier_arrive(mainloop_full_mbar_ptr + stage)
                ab_iter += 1
