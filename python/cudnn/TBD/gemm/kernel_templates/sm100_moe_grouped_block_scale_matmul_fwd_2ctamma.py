        ...
        while is_valid != 0:
            ...
            if is_valid != 0:
                ...
                if group_begin != previous_group_begin:
                    ...
                    for _ai in cutlass.range_constexpr(num_a_operands):
                        if elect_one:
                            row_base = mA_list[_ai].iterator.raw_ptr().toint() + ((group_begin * a_stride_m * ab_dtype.width) >> 3)
                ...
                for k_tile_idx in range(num_k_tiles):
                    ...
                    if a_issue:
                        for _ai in cutlass.range_constexpr(num_a_operands):
                            if nvvm.elect_sync():
                                nvvm.cp_async_bulk_tensor_shared_cluster_global(
                                    smem_a_list[_ai] + sA_elems * stage,
                        ...
                        for _ai in cutlass.range_constexpr(num_a_operands):
                            if nvvm.elect_sync():
                                nvvm.cp_async_bulk_tensor_shared_cluster_global(
                                    smem_sfa_list[_ai] + sfa_smem_bytes * stage,
                    ...
                    if b_issue:
                        for _bj in cutlass.range_constexpr(num_b_operands):
                            if nvvm.elect_sync():
                                nvvm.cp_async_bulk_tensor_shared_cluster_global(
                                    smem_b_list[_bj] + sB_elems * stage,
                        ...
                        for _bj in cutlass.range_constexpr(num_b_operands):
                            if nvvm.elect_sync():
                                nvvm.cp_async_bulk_tensor_shared_cluster_global(
                                    smem_sfb_list[_bj] + sfb_smem_bytes * stage,
