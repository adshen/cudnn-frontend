...
def _l2_swizzle_tile(raw_m, raw_n, nt_m, nt_n, swizzle_w):
    ...
    L2 reuse.
    """
    if cutlass.const_expr(swizzle_w <= 1):
        return raw_m, raw_n
    t = raw_n * nt_m + raw_m
    blk = nt_m * swizzle_w
    sb = t // blk
...
@cute.kernel
def _kernel(
    tma_a_desc: cutlass.GridConstant[_tma.TensorMap],
    tma_b_desc: cutlass.GridConstant[_tma.TensorMap],
    ...
    c_stride_n: cutlass.Int64,
    c_stride_l: cutlass.Int64,
    ...
    if warp_idx == 0:
        if nvvm.elect_sync():
            ...
            for i in range(ab_stages):
                nvvm.mbarrier_init(a_full_mbar_ptr + i, 1)
                nvvm.mbarrier_init(b_full_mbar_ptr + i, 1)
                nvvm.mbarrier_init(mainloop_full_mbar_ptr + i, mainloop_full_count)
            ...
            for i in range(CLC_SCHED_STAGES):
                nvvm.mbarrier_init(clc_full_mbar_ptr + i, 1)
                nvvm.mbarrier_init(clc_empty_mbar_ptr + i, clc_empty_count)
    ...
    num_tmem_cols = acc_stages * cols_per_acc_stage
    ...
    if warp_idx == scheduler_warp_id:
        ...
        while is_valid_sched != 0:
            ...
            if nvvm.elect_sync():
                nvvm.mbarrier_arrive_expect_tx(clc_full_mbar_ptr + stage, 16)
            ...
            while not nvvm.mbarrier_try_wait_parity(clc_full_mbar_ptr + stage, clc_full_phase, time_limit=10_000_000):
                pass
            ...
            if nvvm.elect_sync():
                empty_remote = nvvm.mapa(clc_empty_mbar_ptr + stage, 0)
    ...
    if warp_idx == tma_warp_id:
        ...
        while is_valid != 0:
            ...
            for k_tile_idx in range(num_k_tiles):
                ...
                if cutlass.const_expr(mainloop_a_cast):
                    sA_tma_dst = smem_a_load + sA_elems * stage
                else:
                    sA_tma_dst = sA_stage
                if cutlass.const_expr(mainloop_b_cast):
                    sB_tma_dst = smem_b_load + sB_elems * stage
                else:
                    sB_tma_dst = sB_stage

                if cutlass.const_expr(mainloop_fuse_a):
                    if nvvm.elect_sync():
                        nvvm.mbarrier_arrive_expect_tx(a_full_mbar_ptr + stage, sA_tma_bytes)
                else:
                    if is_pair_leader:
                        if nvvm.elect_sync():
                            nvvm.mbarrier_arrive_expect_tx(a_full_mbar_ptr + stage, sA_tma_bytes * 2)
                if cutlass.const_expr(mainloop_fuse_b):
                    if nvvm.elect_sync():
                        nvvm.mbarrier_arrive_expect_tx(b_full_mbar_ptr + stage, sB_tma_bytes)
                else:
                    if is_pair_leader:
                        if nvvm.elect_sync():
                            nvvm.mbarrier_arrive_expect_tx(b_full_mbar_ptr + stage, sB_tma_bytes * 2)
                if cutlass.const_expr(mainloop_fuse_a):
                        ...
                        if cutlass.const_expr(a_is_m_major):
                            for m_group in cutlass.range_constexpr(cta_tile_mnk[0] // a_tma_group_elems):
                                nvvm.cp_async_bulk_tensor_shared_cluster_global(
                                    sA_tma_dst + m_group * a_tma_group_elems * cgrp_tile_mnk[2],
                                    tma_a_desc.get_ptr(),
                                    (
                    ...
                    if nvvm.elect_sync():
                        if cutlass.const_expr(b_is_n_major):
                            for n_group in cutlass.range_constexpr(cta_tile_mnk[1] // b_tma_group_elems):
                                nvvm.cp_async_bulk_tensor_shared_cluster_global(
                                    sB_tma_dst + n_group * b_tma_group_elems * cgrp_tile_mnk[2],
                                    tma_b_desc.get_ptr(),
                                    (
            ...
            tile_l = l_idx
            if nvvm.elect_sync():
                empty_remote = nvvm.mapa(clc_empty_mbar_ptr + consumer_stage, 0)
                nvvm.mbarrier_arrive(empty_remote, scope=nvvm.MemScope.CLUSTER, relaxed=True)
            tile_iter += 1
        ...
        if is_pair_leader:
            ...
            while is_valid != 0:
                ...
                for k_tile_idx in range(num_k_tiles):
                    ...
                    desc_a_base = cutlass.primitives.Tcgen05SmemDesc.build(
                    ...
                    desc_b_base = cutlass.primitives.Tcgen05SmemDesc.build(
        ...
        while is_valid != 0:
            for k_tile_idx in range(num_k_tiles):
                ...
                if cutlass.const_expr(mainloop_fuse_a):
                    ...
                    sA_stage = smem_a + sA_elems * stage
                    if cutlass.const_expr(mainloop_a_cast):
                        sA_load_stage = smem_a_load + sA_elems * stage
                        ...
                        for chunk in cutlass.range_constexpr(ml_chunks_a):
                            ...
                            ml_kloc = cutlass.full_like(ml_f32_a, cutlass.Float32(ml_L % cgrp_tile_mnk[2]))
                ...
                if cutlass.const_expr(mainloop_fuse_b):
                    ...
                    sB_stage = smem_b + sB_elems * stage
                    if cutlass.const_expr(mainloop_b_cast):
                        sB_load_stage = smem_b_load + sB_elems * stage
                ...
                cute.arch.sync_warp()
                if nvvm.elect_sync():
                    ml_full_remote = nvvm.mapa(mainloop_full_mbar_ptr + stage, pair_leader_rank)
            ...
            for subtile_idx in cutlass.range_constexpr(subtile_cnt):
                ...
                if cutlass.const_expr(cd_out_is_m_major):
                    ...
                    for _h in cutlass.range_constexpr(2):
                        ...
                        vec_f32 = _lv
                        col_j = col
                        linear_idx = tile_l * c_stride_l + row * c_stride_m + col_j * c_stride_n
                        ...
                        if cutlass.const_expr(cd_out_is_m_major):
                            for _mb in cutlass.range_constexpr(cta_tile_mnk[0] // cd_mmajor_atom_m):
                                nvvm.cp_async_bulk_tensor_global_shared_cta(
                                    tma_c_desc.get_ptr(),
                                    smem_subtile_ptr + _mb * (cd_mmajor_atom_m * epi_tile_mn[1]),
...
def _host(
    ...
    c_stride_m = problem_size[4]
    c_stride_n = problem_size[5]
    c_stride_l = problem_size[6]
    ...
    if cutlass.const_expr(a_is_m_major):
        a_tma_tensor = cute.make_tensor(
            ...
            cute.make_layout(
                (m, k_sym, a_batch),
                stride=(1, cute.assume(m, 8), cute.assume(m * k_sym, 8)),
            ),
        )
        ...
        a_tma_tensor = cute.make_tensor(
            ...
            cute.make_layout(
                (m, k_sym, a_batch),
                stride=(cute.assume(k_sym, 8), 1, cute.assume(m * k_sym, 8)),
            ),
        )
    ...
    if cutlass.const_expr(b_is_n_major):
        b_tma_tensor = cute.make_tensor(
            ...
            cute.make_layout(
                (n, k_sym, b_batch),
                stride=(1, cute.assume(n, 8), cute.assume(n * k_sym, 8)),
            ),
        )
        ...
        b_tma_tensor = cute.make_tensor(
            ...
            cute.make_layout(
                (n, k_sym, b_batch),
                stride=(cute.assume(k_sym, 8), 1, cute.assume(n * k_sym, 8)),
            ),
        )
    ...
    tma_a_desc = _tma.create_tensor_map_tiled_from_tensor(
        a_tma_tensor,
        dtype=a_desc_dtype,
        box_dims=a_box_dims,
    ...
    tma_b_desc = _tma.create_tensor_map_tiled_from_tensor(
        b_tma_tensor,
        dtype=b_desc_dtype,
        box_dims=b_box_dims,
...
def compile() -> Callable:
    ...
    fake_a = make_fake_compact_tensor(
        ab_load_a_dtype if mainloop_a_cast else ab_dtype,
        (sym_m, sym_k, sym_a_l),
        stride_order=(0, 1, 2) if a_is_m_major else (1, 0, 2),
        assumed_align=16,
    )
    fake_b = make_fake_compact_tensor(
        ab_load_b_dtype if mainloop_b_cast else ab_dtype,
        (sym_n, sym_k, sym_b_l),
