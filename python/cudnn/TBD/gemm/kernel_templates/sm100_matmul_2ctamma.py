...
def _kernel(
    ...
    while num_tmem_alloc_cols < num_tmem_cols:
        num_tmem_alloc_cols = num_tmem_alloc_cols * 2
...
def _host(
    ...
    for _ in cutlass.range_constexpr(num_b_operands):
        ...
        _stride_idx += 3
    c_stride_m = problem_size[_stride_idx]
    c_stride_n = problem_size[_stride_idx + 1]
    c_stride_l = problem_size[_stride_idx + 2]
...
def compile() -> Callable:
    ...
    for _ in range(num_a_operands):
        sym_a_strides.extend(
            (
                cute.sym_int64(divisibility=ab_stride_elems),
                cute.sym_int64(divisibility=ab_stride_elems),
                cute.sym_int64(divisibility=ab_stride_elems),
            )
        )
