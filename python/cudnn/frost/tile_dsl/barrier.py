...
def arrive_expect_tx(mb, n_bytes, pred=None):
    if cutlass.const_expr(pred is None):
        ...
        nvvm.inline_ptx(
            "mbarrier.arrive.expect_tx.release.cta.shared.b64" " {$w0}, [{$r0}], {$r1};",
            write_only_types=[cutlass.Int64],
            read_only_args=[mb.ir_value(), cutlass.Int32(n_bytes)],
            predicate=pred,
        )
    ...
    if cutlass.const_expr(pred is None):
        ...
        if cutlass.const_expr(cta_group == 1):
            nvvm.inline_ptx(
                ...
                read_only_args=[mb_ir],
                predicate=pred,
            )
        else:
            nvvm.inline_ptx(
                "tcgen05.commit.cta_group::2.mbarrier::arrive::one" ".shared::cluster.multicast::cluster.b64 [{$r0}], {$r1};",
                read_only_args=[mb_ir, mcast_mask.to(cutlass.Int16)],
                predicate=pred,
            )
