...
def _build_spec_map():
    ...
    chain = analyze(_graph_swiglu(2048, 256, 256, 9))
    m = {}
...
def main() -> int:
    ...
    for label in config_names:
        ...
        ms = _time_ms(
            lambda: compiled(_vp_moe_mg(compiled, [(tok, w0), (tok, w1)], offsets, out, scale)), warmup=args.warmup, iters=args.iters, delayed=delayed
        )
