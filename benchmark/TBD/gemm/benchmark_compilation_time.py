...
Run this from the cudnn-frontend repo root:
    python benchmark/TBD/gemm/benchmark_compilation_time.py --case matmul --shape 1,1024,1024,1024 --warmup 2 --iters 5
...
_JIT_TIMER = re.compile(
    r"\[JIT-TIMER\] Function: (\S+) \| Execution Time: ([0-9.]+) (\S+)"
)
_UNIT_TO_MS = {
    "ns": 1e-6,
    "us": 1e-3,
...
class CompilationTiming:
    ...
    def other_python_ms(self) -> float:
        return (
            self.elapsed_ms
            - self.graph_analysis_validation_ms
            - self.codegen_render_ms
            - self.module_import_ms
            - self.mod_compile_ms
        )
...
def _build_graph(case: str, batch: int, m: int, n: int, k: int):
    ...
    if case == "matmul":
        ...
        tokens = batch * m
        token = graph.tensor(
            name="token", dim=[1, tokens, k], stride=[tokens * k, k, 1]
        )
...
def _parse_timing(
    ...
    *,
    ...
    if invalid:
        raise RuntimeError(
            f"expected one timing for each CuTe DSL JIT stage, got {invalid}:\n"
            f"{output}"
        )
...
def _measure(
    case: str, batch: int, m: int, n: int, k: int, count: int, *, verbose: bool
) -> list[CompilationTiming]:
    from cudnn.TBD.gemm import compiler
    ...
    with tempfile.TemporaryDirectory(prefix=f"compile-{case}-") as kernel_cache:
        os.environ.update(
            {
                "CUDNN_GEMM_KERNEL_CACHE": kernel_cache,
        ...
        with redirect_stderr(log):
            ...
            compiler._render_block_scale_template = timed(
                "render", original_block_scale_render
            )
            ...
            try:
                for _ in range(count):
                    ...
                    if stage_times.keys() != expected_stages:
                        raise RuntimeError(
                            "expected one codegen, render, import, and mod.compile "
                            f"call per iteration, got {sorted(stage_times)}"
                        )
                    ...
                    timings.append(
                        _parse_timing(
                            output,
                            elapsed_ms=elapsed_ms,
                            graph_analysis_validation_ms=(
                                stage_starts["generate"] - jit_start
                            )
                            / 1e6,
                            codegen_render_ms=(
                                stage_times["generate"] + stage_times["render"]
                            ),
...
def _parse_shape(value: str) -> tuple[int, int, int, int]:
    ...
    if len(shape) != 4 or any(dim <= 0 for dim in shape):
        raise argparse.ArgumentTypeError(
            "shape must be B,M,N,K with positive dimensions"
        )
    return shape
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            "shape as B,M,N,K; for moe_grouped_matmul, B is groups and M is "
            "tokens per group "
            "(default: 1,1024,1024,1024)"
        ),
    )
    ...
    parser.add_argument(
        "--iters", type=int, default=5, help="measured runs (default: 5)"
    )
    parser.add_argument(
        "--verbose", action="store_true", help="print complete CuTe DSL logs"
    )
