...
FP4 (E2M1, 2-per-byte) inputs dequantized by a per-K-block FP8 E4M3 scale (block
16) inside the MMA. The analyzer pattern-matches two block_scale_dequantize
nodes feeding a matmul and JITs the block-scale kernel. Scale factors use the
F8_128x4 swizzled layout — see to_blocked below.
"""
...
from cudnn.frost.gemm.tile_config import CONFIG_sm100_128x256x128_128x256x32_cluster1x1
...
def main(M: int = 256, N: int = 256, K: int = 512, block_size: int = 16) -> None:
    ...
    torch.manual_seed(0)
    if K % block_size != 0:
        raise ValueError(f"K ({K}) must be a multiple of block_size ({block_size}): one FP8 scale factor covers exactly block_size K-elements.")
    sf_k = K // block_size
    lut = torch.tensor(_E2M1, dtype=torch.float32, device=dev)

    # Packed FP4 data, K-major: A (1, M, K/2) bytes, B stored (N, K) so read K-major.
    ...
    # Runtime: A/B as packed FP4 (float4_e2m1fn_x2), SF in blocked layout.
