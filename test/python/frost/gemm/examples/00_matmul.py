...
FROST is a named engine (``frost_gemm_eng0``) in ``heur_mode.A``'s plan list:
select it for the OSS JIT GEMM, deselect it for native cuDNN — the BF16 case
runs both and asserts they match.
"""
...
def _build_plans(g, *, frost: bool) -> None:
    ...
    (g.select_engines if frost else g.deselect_engines)(["frost_gemm_eng0"])
    g.check_support()
    g.build_plans()
