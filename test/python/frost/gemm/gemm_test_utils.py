...
# 100 <= SM < 120 (see KernelTemplate.sm_lo/sm_hi) — gate on arch, not just
...
LEGACY_RE = re.compile(r"^(CONFIG_sm100_\d+x\d+x\d+_\d+x\d+x\d+_cluster\d+x\d+)_([12])ctamma(_static)?$")
