...
@pytest.mark.L0
@pytest.mark.parametrize("d", [512, 256], ids=["dsv4_d512", "qwen_d256"])
...
def test_sdpa_fwd_dsl_sm100_graph_api(dtype, is_causal, d):
    ...
    # select_engines is strict: check_support/build_plans below raise if the
    # probe rejected this graph, so an ineligible config cannot silently fall
    # back to native cuDNN (needs NV_CUDNN_FE_ENABLE_FROST_ENGINES=1).
    graph.select_engines([engine_name(d)])
    graph.check_support()
    graph.build_plans()
...
_FLAVORS = [512, 256]
_FLAVOR_IDS = ["dsv4_d512", "qwen_d256"]
...
def _run_dsl_graph(q_gpu, k_gpu, v_gpu, *, scale, dtype, sdpa_kwargs, seq_len_kv=None, sink=None):
    ...
    g.select_engines([engine_name(q_gpu.shape[-1])])
    g.check_support()
    g.build_plans()
...
def test_dsl_sm100_thd(dtype, d):
    ...
    g.build_operation_graph()
    g.create_execution_plans([cudnn.heur_mode.A])
    g.select_engines([engine_name(d)])
    g.check_support()
    g.build_plans()
    vp = {tq: q_gpu, tk: k_gpu, tv: v_gpu, o: o_gpu, sq: slq, skv: slk, qro: ro, kro: ro, vro: ro, oro: ro}
    g.execute(vp, torch.empty(1, device=dev, dtype=torch.uint8))
    torch.cuda.synchronize()
