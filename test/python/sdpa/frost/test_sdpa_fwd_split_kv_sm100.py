...
def _api_fp8_case(h_q, h_kv, s_q, s_kv, *, mx, d=128, fp8_dtype=torch.float8_e4m3fn, causal=False, pack_gqa=False, b=1):
...
def test_api_splits_the_fp8_family(mx, d):
    ...
    split, got, unsplit, _ = _api_fp8_case(8, 1, 512, 16384, mx=mx, d=d)
    assert split > 1
...
def test_api_fp8_amax_describes_the_recombined_output(mx, d):
    ...
    split, got, _unsplit, amax = _api_fp8_case(8, 1, 512, 16384, mx=mx, d=d)
    assert split > 1
    true_amax = got.abs().max().item()
