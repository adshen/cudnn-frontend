"""Block-scale multi-GEMM (DualBlockScaleMatmulSiluMul).

Two parallel block-scaled GEMMs that share the dequantized A operand feed one
pointwise epilogue: ``silu(dq(A) @ dq(B0)) * (dq(A) @ dq(B1))``. Covers:
  * analyzer: the single shared ``block_scale_dequantize(A)`` is matched into
    BOTH GEMMs → one distinct A operand (num_a_operands == 1), two B operands.
  * end-to-end numerics on the multi-GEMM-capable block-scale strategies
    (1ctamma CLC + 1ctamma static; cta_group=2 is single-GEMM only
    for now). Dual N is capped at 128 (2 × cta_n + SF must fit 512 TMEM cols).
"""

import pytest
import torch
import cudnn
import cudnn.TBD.gemm  # noqa: F401  installs the recorder
...
def _vp_bs_mg(compiled, gemm_pairs, outs, *aux):
    ...
    slots (the SF travels with its data), + outputs + aux."""
    bd = compiled.binding
...
def test_shared_dequant_dedup():
    ...
    # The single dequant(A) is shared by both GEMMs → ONE distinct A operand.
...
def _dual_bs_runtime(combo, M, N, K):
    ...
    if combo == "nvfp4":
        ...
        sfa, sfb0, sfb1 = (_rand_e8m0((M, sf_k), dev), _rand_e8m0((N, sf_k), dev), _rand_e8m0((N, sf_k), dev))
...
def _run(combo, config_name, M, N, K):
    ...
    # The matmul is exact for nvfp4, but swish uses the FAST approximate
    # intrinsics (cuDNN's swishFwd: __expf / __fdividef), so it carries ~1e-3
    # relative error vs torch's accurate silu — not bit-exact. mx paths also
    # carry fp8/fp16 rounding.
...
@pytest.mark.parametrize(
    ...
    [
        # 1ctamma CLC — dual nvfp4 (bit-exact) + mxfp8.  cta_n=128 → 2×128+SF fits 512.
        ("nvfp4", "CONFIG_sm100_128x128x128_128x128x32_cluster1x1_1ctamma", 256, 128, 512),
        ("mxfp8", "CONFIG_sm100_128x128x128_128x128x32_cluster1x1_1ctamma", 256, 128, 512),
        # A-multicast cluster (SFA mirrors A).
        ("nvfp4", "CONFIG_sm100_128x128x128_128x128x32_cluster1x4_1ctamma", 512, 128, 512),
        # 1ctamma static (no CLC, 1 tile/CTA).
        ("nvfp4", "CONFIG_sm100_128x128x128_128x128x32_cluster1x1_1ctamma_static", 256, 128, 512),
        ("mxfp8", "CONFIG_sm100_128x128x128_128x128x32_cluster1x1_1ctamma_static", 256, 128, 512),
        # 2ctamma CLC (2-CTA MMA pair, cluster2x1; cta_n=128 → 2×128+SF fits 512).
        ("nvfp4", "CONFIG_sm100_128x128x128_128x128x32_cluster2x1_2ctamma", 256, 128, 512),
        ("mxfp8", "CONFIG_sm100_128x128x128_128x128x32_cluster2x1_2ctamma", 256, 128, 512),
        # 2ctamma B-multicast pair (cluster4x1).
        ("nvfp4", "CONFIG_sm100_128x128x128_128x128x32_cluster4x1_2ctamma", 512, 128, 512),
        # 2ctamma static (no CLC, 1 tile/pair).
        ("nvfp4", "CONFIG_sm100_128x128x128_128x128x32_cluster2x1_2ctamma_static", 256, 128, 512),
        ("mxfp8", "CONFIG_sm100_128x128x128_128x128x32_cluster2x1_2ctamma_static", 256, 128, 512),
    ],
)
