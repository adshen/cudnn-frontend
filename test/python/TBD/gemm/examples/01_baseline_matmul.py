"""Example 01: baseline matmul (no fusion).

Builds a graph with the **pure cuDNN frontend API**, hands it to cudnn.TBD.gemm
for analysis + GEMM kernel JIT, runs, and compares to torch.matmul.
...
def main(M: int = 256, N: int = 256, K: int = 128) -> None:
    ...
    print(f"[01] {compiled.chain.summary()}")
    print(f"[01] generated kernel: {compiled.generated_path}")
