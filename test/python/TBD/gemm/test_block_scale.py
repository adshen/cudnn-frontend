...
Two tiers:
  * analyzer pattern-match unit tests — no GPU; assert the FusionChain carries
    a correct BlockScaleSpec for the nvfp4 / mxfp8 graph shapes.
  * end-to-end numerics — needs GPU; nvfp4 vs a torch dequant-matmul reference.
"""
...
def _vp_bs(compiled, a, b, outs, sfa, sfb, *aux):
    """Block-scale single-GEMM variant-pack dict from the binding (A/B packed
    data + SFA/SFB + chain.outputs slot order)."""
    bd = compiled.binding
    outs = list(outs) if isinstance(outs, (list, tuple)) else [outs]
    vp = {bd.a_operands[0]: a, bd.b_operands[0]: b, bd.sfa_operands[0]: sfa, bd.sfb_operands[0]: sfb}
...
def _build_nvfp4_graph(
    M, N, K, block_size=16, sf_dt=cudnn.data_type.FP8_E4M3, a_dt=cudnn.data_type.FP4_E2M1, b_dt=None, a_major="k", b_major="k", reorder=True, out_major="n"
):
    sf_k = K // block_size
    ...
    # A: K-major (K contiguous, stride[-1]=1) or M-major (M contiguous, stride[-2]=1).
    a_stride = [M * K, K, 1] if a_major == "k" else [M * K, 1, M]
    # B logical (K, N): K-major (K contiguous, stride[-2]=1) or N-major (N contiguous).
...
def _block_quant_ref(x, block_size, out_dtype, scale_dtype):
    ...
    if scale_dtype is torch.float8_e8m0fnu:
        # The reference impl uses round-toward-positive-infinity for E8M0 scale factors.
...
def test_block_scale_gate_accepts_supported(a_dt, sf_dt, b_dt, bs):
    ...
    _check_block_scale_supported(chain)  # must not raise
...
def test_block_scale_gate_rejects_mismatches():
    ...
    # nvfp4 (fp4+e4m3) with block32 — no supported case has that combo.
    with pytest.raises(NotImplementedError, match="does not support"):
        _check_block_scale_supported(analyze(_build_nvfp4_graph(256, 256, 512, block_size=32, sf_dt=_DT_E4M3)))
    # mixed FP4 A / FP8 B (cross-family) — not a supported case.
    with pytest.raises(NotImplementedError, match="does not support"):
        _check_block_scale_supported(analyze(_build_nvfp4_graph(256, 256, 512, block_size=32, sf_dt=_DT_E8M0, a_dt=_DT_FP4, b_dt=_DT_E4M3)))
...
def test_analyze_detects_nvfp4_block_scale():
    ...
    # Per-side block-scale info (SF tensors are runtime-positional, not stored
    # as TensorRefs — same as the A/B data operands).
...
def test_analyze_detects_mxfp8_block_scale():
    chain = analyze(_build_nvfp4_graph(128, 256, 256, block_size=32, sf_dt=cudnn.data_type.FP8_E8M0, a_dt=cudnn.data_type.FP8_E4M3))
    bs = chain.block_scale
...
def _rand_e8m0(shape, dev):
    # E8M0 holds a power-of-2; small exponents around bias 127 keep the
    # FP32 reference well within range.
...
def _run_bs_numeric(combo, config_name, M, N, K, out_major="n"):
    ...
    compiled(_vp_bs(compiled, a_rt, b_rt, c, _to_blocked(sfa_log).view(1, M, sf_k), _to_blocked(sfb_log).view(1, N, sf_k)))
    torch.cuda.synchronize()

    a_s = a_deq * sfa_log.float().repeat_interleave(bs, 1)
    b_s = b_deq * sfb_log.float().repeat_interleave(bs, 1)
    ref = (a_s @ b_s.t()).to(torch.float16)
    # nvfp4 is bit-exact; mx paths carry fp16 rounding -> small tolerance.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("nvfp4", "CONFIG_sm100_128x256x128_128x256x32_cluster1x1_1ctamma", 256, 256, 512),  # acc_stages=1
        ("nvfp4", "CONFIG_sm100_128x128x128_128x128x32_cluster1x1_1ctamma", 128, 128, 256),  # acc_stages=2
        # mxfp4 (fp4 + e8m0 scale, block32).
        ("mxfp4", "CONFIG_sm100_128x256x128_128x256x32_cluster1x1_1ctamma", 256, 256, 512),
        # mxfp8 (fp8 e4m3 + e8m0 scale, block32) — multi N-block + single N-block.
        ("mxfp8", "CONFIG_sm100_128x256x128_128x256x32_cluster1x1_1ctamma", 256, 512, 512),
        ("mxfp8", "CONFIG_sm100_128x128x128_128x128x32_cluster1x1_1ctamma", 128, 128, 256),
        # Multicast cluster configs — validate SF multicast (SFA mirrors A,
        # SFB mirrors B). Shape 512x512x512 keeps every CTA in the cluster active.
        ("nvfp4", "CONFIG_sm100_128x128x128_128x128x32_cluster1x4_1ctamma", 512, 512, 512),  # A-multicast x4
        ("nvfp4", "CONFIG_sm100_128x256x128_128x256x32_cluster2x2_1ctamma", 512, 512, 512),  # A+B multicast
        ("nvfp4", "CONFIG_sm100_128x256x128_128x256x32_cluster4x1_1ctamma", 512, 512, 512),  # B-multicast x4
        ("mxfp8", "CONFIG_sm100_128x256x128_128x256x32_cluster2x2_1ctamma", 512, 512, 512),  # mx + A+B multicast
        # 2-CTA MMA pair (cta_group=2): 2-SM utccp + 2-SM block-scale MMA. cta_n=128
        # → non-overlap acc (acc_stages=2); cta_n=256 → acc-overlap (acc_stages=1 +
        # column toggle, own-epilogue dealloc rendezvous).
        ("nvfp4", "CONFIG_sm100_128x128x128_128x128x32_cluster2x1_2ctamma", 256, 128, 512),  # non-overlap
        ("nvfp4", "CONFIG_sm100_128x256x128_128x256x32_cluster2x1_2ctamma", 256, 256, 512),  # acc-overlap
        ("mxfp8", "CONFIG_sm100_128x256x128_128x256x32_cluster2x1_2ctamma", 256, 256, 512),  # mx + overlap
        ("nvfp4", "CONFIG_sm100_128x256x128_128x256x32_cluster4x1_2ctamma", 512, 512, 512),  # B-mcast pair
        ("nvfp4", "CONFIG_sm100_128x256x128_128x256x32_cluster4x2_2ctamma", 512, 512, 512),  # A+B-mcast pair
        ("mxfp8", "CONFIG_sm100_128x256x128_128x256x32_cluster4x2_2ctamma", 512, 512, 512),  # mx + A+B pair
        # Static scheduler (no CLC, 1 tile/CTA) — small ≤1-wave shapes. cluster1x1,
        # acc_stages=2 (cta_n=128) and acc-overlap (cta_n=256).
        ("nvfp4", "CONFIG_sm100_128x128x128_128x128x32_cluster1x1_1ctamma_static", 128, 128, 256),
        ("nvfp4", "CONFIG_sm100_128x256x128_128x256x32_cluster1x1_1ctamma_static", 256, 256, 512),  # acc-overlap
        ("mxfp8", "CONFIG_sm100_128x128x128_128x128x32_cluster1x1_1ctamma_static", 256, 256, 256),
        # 2-CTA pair static (no CLC, 1 tile/pair). cta_n=128 non-overlap + cta_n=256 overlap.
        ("nvfp4", "CONFIG_sm100_128x128x128_128x128x32_cluster2x1_2ctamma_static", 256, 128, 512),  # non-overlap
        ("nvfp4", "CONFIG_sm100_128x256x128_128x256x32_cluster2x1_2ctamma_static", 256, 256, 512),  # acc-overlap
        ("mxfp8", "CONFIG_sm100_128x256x128_128x256x32_cluster2x1_2ctamma_static", 256, 256, 512),  # mx + overlap
    ],
)
...
@pytest.mark.parametrize(
    "combo,config_name,M,N,K",
    [
        ("nvfp4", "CONFIG_sm100_128x128x128_128x128x32_cluster1x1_1ctamma", 256, 256, 256),
        ("nvfp4", "CONFIG_sm100_128x256x128_128x256x32_cluster1x1_1ctamma_static", 256, 256, 512),
        ("mxfp8", "CONFIG_sm100_128x256x128_128x256x32_cluster2x1_2ctamma", 256, 256, 512),
        ("nvfp4", "CONFIG_sm100_128x256x128_128x256x32_cluster2x1_2ctamma_static", 256, 256, 512),
    ],
)
def test_block_scale_m_major(combo, config_name, M, N, K):
    """M-major block-scale output across dynamic/static 1-CTA and 2-CTA templates."""
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("mxfp8", "CONFIG_sm100_128x128x128_128x128x32_cluster1x1_1ctamma_static", "zero_stride"),
    ],
)
...
def _run_bs_reduction_numeric(combo, config_name, M, N, K, mode, red_dims, red_stride, ref_dims):
    ...
    compiled(_vp_bs(compiled, a_rt, b_rt, [c_term, c_red], _to_blocked(sfa_log).view(1, M, K // bs), _to_blocked(sfb_log).view(1, N, K // bs)))
    torch.cuda.synchronize()
...
@pytest.mark.parametrize(
    "combo,config_name,M,N,K",
    [
        ("nvfp4", "CONFIG_sm100_128x128x128_128x128x32_cluster1x1_1ctamma", 128, 128, 256),
        ("nvfp4", "CONFIG_sm100_128x256x128_128x256x32_cluster2x1_2ctamma", 256, 256, 512),
        ("nvfp4", "CONFIG_sm100_128x256x128_128x256x32_cluster4x2_2ctamma", 512, 512, 512),
    ],
)
...
@pytest.mark.parametrize(
    "config_name,M,N,K",
    [
        ("CONFIG_sm100_128x128x128_128x128x32_cluster1x1_1ctamma_static", 128, 128, 256),
        ("CONFIG_sm100_128x256x128_128x256x32_cluster2x1_2ctamma_static", 256, 256, 512),
    ],
)
...
def test_nvfp4_oob_shape(config_name):
    """nvfp4 -> bf16 on a deliberately awkward shape: M=23 (M-OOB), N=56 (N-OOB,
    not 32-byte aligned), K=736 (K-OOB, not a cta_k multiple; sf_k=46 -> SF
    padded to 128x128x48). Exercises ceil-padded SF descriptors + OOB."""
    dev = "cuda"
...
# The scale-factor layout is unchanged — only the packed-data descriptor flips.
# ---------------------------------------------------------------------------
@_GPU
@pytest.mark.parametrize(
    "config_name,M,N,K",
    [
        ...
        ("CONFIG_sm100_128x256x128_128x256x32_cluster1x1_1ctamma", 256, 512, 512),  # B has 2 N-groups
        ("CONFIG_sm100_128x256x128_128x256x32_cluster2x1_2ctamma", 256, 256, 512),  # 2-CTA + overlap
    ],
)
def test_mxfp8_m_major_a_n_major_b(config_name, M, N, K):
    ...
    # K-major reference data, then re-laid-out so A is M-contiguous and B is
    # N-contiguous (same logical values → same reference).
    a_rt = (torch.randn(1, M, K, device=dev) * 0.5).to(torch.float8_e4m3fn)
    b_rt = (torch.randn(1, N, K, device=dev) * 0.5).to(torch.float8_e4m3fn)
    a_deq = a_rt.float()[0]
    b_deq = b_rt.float()[0]
...
def test_fp4_rejects_non_k_major():
    """FP4 (nvfp4 / mxfp4) must be K-major — the sub-byte packing mis-strides an
    M/N-major descriptor, so the compiler rejects it at JIT time."""
    M = N = K = 256
    for a_major, b_major in (("m", "k"), ("k", "n")):
        g = _build_nvfp4_graph(M, N, K, block_size=16, sf_dt=cudnn.data_type.FP8_E4M3, a_dt=cudnn.data_type.FP4_E2M1, a_major=a_major, b_major=b_major)
